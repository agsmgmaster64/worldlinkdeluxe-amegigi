...
static u32 AI_CalcMoveEffectScore(u32 battlerAtk, u32 battlerDef, u32 move)
{
    ...
    case EFFECT_ATTACK_SPATK_UP:    // work up
        ...
        break;
    case EFFECT_ROTOTILLER:
            if (IS_BATTLER_OF_TYPE(battlerAtk, TYPE_NATURE) && IsBattlerGrounded(battlerAtk))
            {
                ADJUST_SCORE(IncreaseStatUpScore(battlerAtk, battlerDef, STAT_CHANGE_ATK));
                ADJUST_SCORE(IncreaseStatUpScore(battlerAtk, battlerDef, STAT_CHANGE_SPATK));
            }
            if (hasPartner && IS_BATTLER_OF_TYPE(BATTLE_PARTNER(battlerAtk), TYPE_NATURE) && IsBattlerGrounded(BATTLE_PARTNER(battlerAtk)))
            {
                ADJUST_SCORE(IncreaseStatUpScore(BATTLE_PARTNER(battlerAtk), battlerDef, STAT_CHANGE_ATK));
                ADJUST_SCORE(IncreaseStatUpScore(BATTLE_PARTNER(battlerAtk), battlerDef, STAT_CHANGE_SPATK));
            }
            if (IS_BATTLER_OF_TYPE(FOE(battlerAtk), TYPE_NATURE) && IsBattlerGrounded(FOE(battlerAtk)))
            {
            ...
            if (IS_BATTLER_OF_TYPE(BATTLE_PARTNER(FOE(battlerAtk)), TYPE_NATURE) && IsBattlerGrounded(BATTLE_PARTNER(FOE(battlerAtk))))
            {
    ...
    case EFFECT_FLOWER_SHIELD:
        ...
        if (IS_BATTLER_OF_TYPE(FOE(battlerAtk), TYPE_NATURE))
        {
            if (aiData->abilities[FOE(battlerAtk)] == ABILITY_CONTRARY)
                ADJUST_SCORE(WEAK_EFFECT);
            else
        ...
        if (IS_BATTLER_OF_TYPE(BATTLE_PARTNER(FOE(battlerAtk)), TYPE_NATURE))
        {
            if (aiData->abilities[BATTLE_PARTNER(FOE(battlerAtk))] == ABILITY_CONTRARY)
                ADJUST_SCORE(WEAK_EFFECT);
            else
