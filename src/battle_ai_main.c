...
// the following checks apply to any target (including user)
        ...
        case EFFECT_ACCURACY_DOWN_2:
            if (!ShouldLowerStat(battlerDef, aiData->abilities[battlerDef], STAT_ACC))
                ADJUST_SCORE(-10);
                        ...
                        || (B_ILLUMINATE_EFFECT >= GEN_9 && aiData->abilities[battlerDef] == ABILITY_DIVA))
                ADJUST_SCORE(-8);
            break;
    ...
    // other
        ...
        case EFFECT_HOLY_TERRAIN:
            if (PartnerMoveEffectIsTerrain(BATTLE_PARTNER(battlerAtk), aiData->partnerMove) || gFieldStatuses & STATUS_FIELD_HOLY_TERRAIN)
                ADJUST_SCORE(-10);
            break;
        case EFFECT_PLEDGE:
        ...
        case EFFECT_THIRD_TYPE:
            if (IS_BATTLER_OF_TYPE(battlerDef, gMovesInfo[move].argument) || PartnerMoveIsSameAsAttacker(BATTLE_PARTNER(battlerAtk), battlerDef, move, aiData->partnerMove) || GetActiveGimmick(battlerDef) == GIMMICK_TERA)
                ADJUST_SCORE(-10);
            break;
...
static s32 AI_DoubleBattle(u32 battlerAtk, u32 battlerDef, u32 move, s32 score)
{
    ...
    if (aiData->partnerMove != 0)
    {
        ...
        case EFFECT_PERISH_SONG:
            ...
            {
                if (IsTrappingMove(aiData->partnerMove) || gMovesInfo[predictedMove].effect == EFFECT_INGRAIN)
                ...
                if (aiData->abilities[battlerAtk] != aiData->abilities[BATTLE_PARTNER(battlerAtk)] && !attackerHasBadAbility)
                {
                    ...
                    if (aiData->abilities[BATTLE_PARTNER(battlerAtk)] == ABILITY_FRETFUL)
                    {
                        ADJUST_SCORE(10);
                          ...
                          || aiData->abilities[BATTLE_PARTNER(battlerAtk)] == ABILITY_FASCINATE)
                    {
                        ADJUST_SCORE(DECENT_EFFECT);
                    }
                    // Active mon abilities
                    if (aiData->abilities[battlerAtk] == ABILITY_FOCUS
...
static u32 AI_CalcMoveEffectScore(u32 battlerAtk, u32 battlerDef, u32 move)
{
    ...
    case EFFECT_SEMI_INVULNERABLE:
        ...
        break;
// the waters are still unclear on what's going on here
// i just don't quite get how exactly i should approach this
// if i should even get involved at all
    case EFFECT_FIRST_TURN_ONLY:
    ...
    case EFFECT_HEAL_BLOCK:
        ...
        break;
    case EFFECT_SOAK:
        if (HasMoveWithType(battlerAtk, TYPE_WIND) || HasMoveWithType(battlerAtk, TYPE_NATURE) || (HasMoveEffect(battlerAtk, EFFECT_SUPER_EFFECTIVE_ON_ARG) && gMovesInfo[move].argument == TYPE_WATER) )
    ...
    case EFFECT_MAGNET_RISE:
          ...
          && !(AI_GetTypeEffectiveness(MOVE_EARTHQUAKE, battlerDef, battlerAtk) == AI_EFFECTIVENESS_x0)) // Doesn't resist ground move
        {
           ...
           {
                if (gMovesInfo[predictedMove].type == TYPE_EARTH)
...
static s32 AI_PreferBatonPass(u32 battlerAtk, u32 battlerDef, u32 move, s32 score)
{
    ...
    case EFFECT_AQUA_RING:
        ...
        break;
    case EFFECT_PROTECT:
        if (gLastMoves[battlerAtk] == MOVE_DETECT)
            ADJUST_SCORE(-2);
        else
            ADJUST_SCORE(DECENT_EFFECT);
        break;
