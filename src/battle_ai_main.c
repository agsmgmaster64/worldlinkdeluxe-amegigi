    ...
    if (!(moveTarget & MOVE_TARGET_USER))
    {
        ...
        if (!DoesBattlerIgnoreAbilityChecks(aiData->abilities[battlerAtk], move))
        {
            ...
            if (isDoubleBattle)
            {
                ...
                {
                case ABILITY_LIGHTNING_ROD:
                    if (moveType == TYPE_WIND && !IsMoveRedirectionPrevented(move, aiData->abilities[battlerAtk]))
                        RETURN_SCORE_MINUS(20);
                    break;
                case ABILITY_STORM_DRAIN:
...
// the following checks apply to any target (including user)
    ...
    // stat lowering effects
        ...
        case EFFECT_ATTACK_DOWN_2:
            if (!ShouldLowerStat(battlerDef, aiData->abilities[battlerDef], STAT_ATK)) //|| !HasMoveWithSplit(battlerDef, SPLIT_PHYSICAL))
                ADJUST_SCORE(-10);
            else if (aiData->abilities[battlerDef] == ABILITY_HIGH_STRENGTH)
                ADJUST_SCORE(-10);
            break;
        ...
        case EFFECT_ACCURACY_DOWN_2:
            if (!ShouldLowerStat(battlerDef, aiData->abilities[battlerDef], STAT_ACC))
                ADJUST_SCORE(-10);
            else if (aiData->abilities[battlerDef] == ABILITY_KEEN_EYE || (B_ILLUMINATE_EFFECT >= GEN_9 && aiData->abilities[battlerDef] == ABILITY_DIVA))
                ADJUST_SCORE(-8);
            break;
    ...
    // other
        ...
        case EFFECT_LOW_KICK:
            ...
            if (aiData->abilities[battlerDef] == ABILITY_PLAY_GHOST && effectiveness < AI_EFFECTIVENESS_x2)
                ADJUST_SCORE(-10);
            break;
        case EFFECT_COUNTER:
        case EFFECT_MIRROR_COAT:
        ...
        case EFFECT_WATER_SPORT:
            ...
            break;
        case EFFECT_ABSORB:
            if (aiData->abilities[battlerDef] == ABILITY_STRANGE_MIST)
                ADJUST_SCORE(-6);
            break;
        ...
        case EFFECT_RECHARGE:
            if (aiData->abilities[battlerDef] == ABILITY_PLAY_GHOST && effectiveness < AI_EFFECTIVENESS_x2)
                ADJUST_SCORE(-10);
            else if (aiData->abilities[battlerAtk] != ABILITY_FRETFUL
        ...
        case EFFECT_THIRD_TYPE:
            ...
            case MOVE_FORESTS_CURSE:
                if (IS_BATTLER_OF_TYPE(battlerDef, TYPE_NATURE) || PartnerMoveIsSameAsAttacker(BATTLE_PARTNER(battlerAtk), battlerDef, move, aiData->partnerMove))
                    ADJUST_SCORE(-10);
                break;
            }
            break;
    ...
    if (IS_TARGETING_PARTNER(battlerAtk, battlerDef))
    {
        ...
        if (!partnerProtecting)
        {
            ...
            case EFFECT_SKILL_SWAP:
                ...
                {
                    if (aiData->abilities[BATTLE_PARTNER(battlerAtk)] == ABILITY_FRETFUL)
                    {
                        RETURN_SCORE_PLUS(10);
                    }
                    else if (aiData->abilities[battlerAtk] == ABILITY_FOCUS
...
// stat lowering effects
    ...
    case EFFECT_LOCK_ON:
        if (HasMoveEffect(battlerAtk, EFFECT_OHKO))
            ADJUST_SCORE(3);
        else if (aiData->abilities[battlerAtk] == ABILITY_FOCUS && HasMoveWithLowAccuracy(battlerAtk, battlerDef, 80, TRUE, aiData->abilities[battlerAtk], aiData->abilities[battlerDef], aiData->holdEffects[battlerAtk], aiData->holdEffects[battlerDef]))
            ADJUST_SCORE(3);
