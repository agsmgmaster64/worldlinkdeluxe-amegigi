    ...
    // other
        ...
        case EFFECT_THIRD_TYPE:
            ...
            case MOVE_FORESTS_CURSE:
                if (IS_BATTLER_OF_TYPE(battlerDef, TYPE_NATURE) || PartnerMoveIsSameAsAttacker(BATTLE_PARTNER(battlerAtk), battlerDef, move, aiData->partnerMove) || IsTerastallized(battlerDef))
                    ADJUST_SCORE(-10);
                break;
            }
            break;
...
static u32 AI_CalcMoveScore(u32 battlerAtk, u32 battlerDef, u32 move)
{
    ...
    case EFFECT_HAZE:
        ...
        break;
    case EFFECT_ROAR:
        if ((gMovesInfo[move].soundMove && aiData->abilities[battlerDef] == ABILITY_SOUNDPROOF) || aiData->abilities[battlerDef] == ABILITY_GATE_KEEPER)
            break;
        score += AI_TryToClearStats(battlerAtk, battlerDef, isDoubleBattle);
        break;
