...
static u8 ChooseMoveOrAction_Singles(void)
{
        ...
        && AI_THINKING_STRUCT->aiFlags & (AI_FLAG_CHECK_VIABILITY | AI_FLAG_CHECK_BAD_MOVE | AI_FLAG_TRY_TO_FAINT | AI_FLAG_PREFER_BATON_PASS))
    {
        ...
        if (GetBattlerAbility(sBattler_AI) == ABILITY_FRETFUL
    ...
    if (!(moveTarget & MOVE_TARGET_USER))
    {
        ...
        if (!DoesBattlerIgnoreAbilityChecks(AI_DATA->abilities[battlerAtk], move))
        {
            ...
            case ABILITY_FLOWER_VEIL:
                if (IS_BATTLER_OF_TYPE(battlerDef, TYPE_NATURE) && (IsNonVolatileStatusMoveEffect(moveEffect) || IsStatLoweringMoveEffect(moveEffect)))
                    RETURN_SCORE_MINUS(10);
                break;
            ...
            if (isDoubleBattle)
            {
                ...
                {
                case ABILITY_LIGHTNING_ROD:
                    if (moveType == TYPE_WIND && !IsMoveRedirectionPrevented(move, AI_DATA->abilities[battlerAtk]))
                        RETURN_SCORE_MINUS(20);
                    break;
                case ABILITY_STORM_DRAIN:
...
// the following checks apply to any target (including user)
    ...
    // stat lowering effects
        ...
        case EFFECT_ATTACK_DOWN_2:
            if (!ShouldLowerStat(battlerDef, AI_DATA->abilities[battlerDef], STAT_ATK)) //|| !HasMoveWithSplit(battlerDef, SPLIT_PHYSICAL))
                score -= 10;
            else if (AI_DATA->abilities[battlerDef] == ABILITY_HIGH_STRENGTH)
                score -= 10;
            break;
    ...
    // other
        ...
        case EFFECT_LOW_KICK:
            ...
            if (AI_DATA->abilities[battlerDef] == ABILITY_PLAY_GHOST && effectiveness < AI_EFFECTIVENESS_x2)
                score -= 10;
            break;
        case EFFECT_COUNTER:
        case EFFECT_MIRROR_COAT:
        ...
        case EFFECT_ROAR:
            if (CountUsablePartyMons(battlerDef) == 0)
                score -= 10;
            else if (AI_DATA->abilities[battlerDef] == ABILITY_GATE_KEEPER)
                score -= 10;
            break;
        ...
        case EFFECT_LEECH_SEED:
            if (gStatuses3[battlerDef] & STATUS3_LEECHSEED
             ...
             || DoesPartnerHaveSameMoveEffect(BATTLE_PARTNER(battlerAtk), battlerDef, move, AI_DATA->partnerMove))
                score -= 10;
            else if (AI_DATA->abilities[battlerDef] == ABILITY_STRANGE_MIST)
                score -= 3;
            break;
        case EFFECT_DISABLE:
        ...
        case EFFECT_KNOCK_OFF:
            if (AI_DATA->abilities[battlerDef] == ABILITY_COLLECTOR)
                score -= 10;
            break;
        case EFFECT_INGRAIN:
        ...
        case EFFECT_WATER_SPORT:
            ...
            break;
        case EFFECT_ABSORB:
            if (AI_DATA->abilities[battlerDef] == ABILITY_STRANGE_MIST)
                score -= 6;
            break;
        ...
        case EFFECT_FLOWER_SHIELD:
            if (!IS_BATTLER_OF_TYPE(battlerAtk, TYPE_NATURE)
              && !(isDoubleBattle && IS_BATTLER_OF_TYPE(BATTLE_PARTNER(battlerAtk), TYPE_NATURE)))
                score -= 10;
            break;
        ...
        case EFFECT_RECHARGE:
            if (AI_DATA->abilities[battlerDef] == ABILITY_PLAY_GHOST && effectiveness < AI_EFFECTIVENESS_x2)
                score -= 10;
            else if (AI_DATA->abilities[battlerAtk] != ABILITY_FRETFUL
        ...
        case EFFECT_LASER_FOCUS:
            if (gStatuses3[battlerAtk] & STATUS3_LASER_FOCUS)
                score -= 10;
            else if (AI_DATA->abilities[battlerDef] == ABILITY_SHELL_ARMOR || AI_DATA->abilities[battlerDef] == ABILITY_GUARD_ARMOR)
                score -= 8;
            break;
        case EFFECT_SKETCH:
            if (gLastMoves[battlerDef] == MOVE_NONE)
        ...
        case EFFECT_DOUBLE_SHOCK:
            if (!IS_BATTLER_OF_TYPE(battlerAtk, TYPE_WIND))
                score -= 10;
            break;
        case EFFECT_DEFOG:
        ...
        case EFFECT_THIRD_TYPE:
            ...
            case MOVE_FORESTS_CURSE:
                if (IS_BATTLER_OF_TYPE(battlerDef, TYPE_NATURE) || PartnerMoveIsSameAsAttacker(BATTLE_PARTNER(battlerAtk), battlerDef, move, AI_DATA->partnerMove))
                    score -= 10;
                break;
            }
            break;
        ...
        case EFFECT_ELECTRIFY:
            if (AI_WhoStrikesFirst(battlerAtk, battlerDef, move) == AI_IS_SLOWER
              //|| GetMoveTypeSpecial(battlerDef, predictedMove) == TYPE_WIND // Move will already be electric type
              || PartnerMoveIsSameAsAttacker(BATTLE_PARTNER(battlerAtk), battlerDef, move, AI_DATA->partnerMove))
                score -= 10;
            break;
...
static s16 AI_DoubleBattle(u8 battlerAtk, u8 battlerDef, u16 move, s16 score)
{
    ...
    if (IsTargetingPartner(battlerAtk, battlerDef))
    {
        if (GetMoveDamageResult(move) == MOVE_POWER_OTHER)
        {
            ...
            if (!partnerProtecting && moveTarget != MOVE_TARGET_BOTH && !DoesBattlerIgnoreAbilityChecks(AI_DATA->abilities[battlerAtk], move))
            {
                ...
                case ABILITY_MOTOR_DRIVE:
                    if (moveType == TYPE_WIND && BattlerStatCanRise(battlerAtkPartner, atkPartnerAbility, STAT_SPEED))
                    {
                        RETURN_SCORE_PLUS(1);
                    }
                    break;
        ...
        if (!partnerProtecting)
        {
            ...
            case EFFECT_SKILL_SWAP:
                ...
                {
                    if (AI_DATA->abilities[BATTLE_PARTNER(battlerAtk)] == ABILITY_FRETFUL)
                    {
                        RETURN_SCORE_PLUS(10);
                    }
                    else if (AI_DATA->abilities[battlerAtk] == ABILITY_FOCUS
    ...
    else // checking opponent
    {
        ...
        {
        case EFFECT_SKILL_SWAP:
            if (AI_DATA->abilities[battlerAtk] == ABILITY_FRETFUL)
                score += 5;
        ...
        case EFFECT_MAGNITUDE:
            if (!IsBattlerGrounded(battlerAtkPartner)
             || (IsBattlerGrounded(battlerAtkPartner)
              ...
              || IS_BATTLER_OF_TYPE(battlerAtkPartner, TYPE_WIND)
              || IS_BATTLER_OF_TYPE(battlerAtkPartner, TYPE_MIASMA)
              || IS_BATTLER_OF_TYPE(battlerAtkPartner, TYPE_BEAST))
...
// stat lowering effects
    ...
    case EFFECT_REST:
        ...
        else if (ShouldRecover(battlerAtk, battlerDef, move, 100))
        {
            if (AI_DATA->holdEffects[battlerAtk] == HOLD_EFFECT_CURE_SLP
              ...
              || AI_DATA->abilities[battlerAtk] == ABILITY_MAINTENANCE
    ...
    case EFFECT_LEECH_SEED:
        if (IS_BATTLER_OF_TYPE(battlerDef, TYPE_NATURE)
          ...
          || AI_DATA->abilities[battlerDef] == ABILITY_STRANGE_MIST
    ...
    case EFFECT_LOCK_ON:
        if (HasMoveEffect(battlerAtk, EFFECT_OHKO))
            score += 3;
        else if (AI_DATA->abilities[battlerAtk] == ABILITY_FOCUS && HasMoveWithLowAccuracy(battlerAtk, battlerDef, 80, TRUE, AI_DATA->abilities[battlerAtk], AI_DATA->abilities[battlerDef], AI_DATA->holdEffects[battlerAtk], AI_DATA->holdEffects[battlerDef]))
            score += 3;
    ...
    case EFFECT_THIEF:
        {
            ...
            if (canSteal && AI_DATA->items[battlerAtk] == ITEM_NONE
              ...
              && AI_DATA->abilities[battlerDef] != ABILITY_COLLECTOR)
            {
    ...
    case EFFECT_BESTOW:
        ...
        case HOLD_EFFECT_FLAME_ORB:
            ...
            break;
        case HOLD_EFFECT_BLACK_SLUDGE:
            if (!IS_BATTLER_OF_TYPE(battlerDef, TYPE_MIASMA) && AI_DATA->abilities[battlerDef] != ABILITY_MAGIC_GUARD)
                score += 3;
            break;
        case HOLD_EFFECT_IRON_BALL:
        ...
        default:
            if (move != MOVE_BESTOW && AI_DATA->items[battlerAtk] == ITEM_NONE)
            {
                ...
                case HOLD_EFFECT_FLAME_ORB:
                    ...
                    break;
                case HOLD_EFFECT_BLACK_SLUDGE:
                    if (IS_BATTLER_OF_TYPE(battlerAtk, TYPE_MIASMA) || AI_DATA->abilities[battlerAtk] == ABILITY_MAGIC_GUARD)
                        score += 3;
                    break;
                case HOLD_EFFECT_IRON_BALL:
                    if (HasMoveEffect(battlerAtk, EFFECT_FLING))
    ...
    case EFFECT_BUG_BITE:   // And pluck
        if (gBattleMons[battlerDef].status2 & STATUS2_SUBSTITUTE || AI_DATA->abilities[battlerDef] == ABILITY_COLLECTOR)
            break;
    ...
    case EFFECT_ION_DELUGE:
        if ((AI_DATA->abilities[battlerAtk] == ABILITY_VOLT_ABSORB
          ...
          && gBattleMoves[predictedMove].type == TYPE_ILLUSION)
            score += 2;
        break;
    case EFFECT_FLING:
        /* TODO
    ...
    case EFFECT_HEAL_BLOCK:
        if (AI_WhoStrikesFirst(battlerAtk, battlerDef, move) == AI_IS_FASTER && predictedMove != MOVE_NONE && IsHealingMoveEffect(gBattleMoves[predictedMove].effect))
          ...
          || (AI_DATA->holdEffects[battlerDef] == HOLD_EFFECT_BLACK_SLUDGE && IS_BATTLER_OF_TYPE(battlerDef, TYPE_MIASMA)))
            score += 2;
        break;
    case EFFECT_SOAK:
        if (HasMoveWithType(battlerAtk, TYPE_WIND) || HasMoveWithType(battlerAtk, TYPE_NATURE) || HasMoveEffect(battlerAtk, EFFECT_FREEZE_DRY))
            score += 2; // Get some super effective moves
        break;
    case EFFECT_THIRD_TYPE:
        if (AI_DATA->abilities[battlerDef] == ABILITY_PLAY_GHOST)
    ...
    case EFFECT_LUCKY_CHANT:
        ...
        break;
    case EFFECT_MAGNET_RISE:
        if (IsBattlerGrounded(battlerAtk) && HasDamagingMoveOfType(battlerDef, TYPE_WIND)
          && !(AI_GetTypeEffectiveness(MOVE_EARTHQUAKE, battlerDef, battlerAtk) == AI_EFFECTIVENESS_x0)) // Doesn't resist ground move
        {
            if (AI_WhoStrikesFirst(battlerAtk, battlerDef, move) == AI_IS_FASTER) // Attacker goes first
           {
                if (gBattleMoves[predictedMove].type == TYPE_EARTH)
...
static s16 AI_HPAware(u8 battlerAtk, u8 battlerDef, u16 move, s16 score)
{
    ...
    if (IsTargetingPartner(battlerAtk, battlerDef))
    {
        if ((effect == EFFECT_HEAL_PULSE || effect == EFFECT_HIT_ENEMY_HEAL_ALLY)
         || (moveType == TYPE_WIND && AI_DATA->abilities[BATTLE_PARTNER(battlerAtk)] == ABILITY_VOLT_ABSORB)
