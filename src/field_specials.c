...
void IsGrassTypeInParty(void)
{
    ...
    for (i = 0; i < PARTY_SIZE; i++)
    {
        ...
        if (GetMonData(pokemon, MON_DATA_SANITY_HAS_SPECIES) && !GetMonData(pokemon, MON_DATA_IS_EGG))
        {
            ...
            if (gSpeciesInfo[species].types[0] == TYPE_NATURE || gSpeciesInfo[species].types[1] == TYPE_NATURE)
            {
...
{
    return GetMonData(&gPlayerParty[GetLeadMonIndex()], MON_DATA_NUZLOCKE_RIBBON, NULL);
}
...
{
    u16 monData;
    CreateMon(&gPlayerParty[0], SPECIES_CHIBI_CHEN, 7, USE_RANDOM_IVS, FALSE, 0, OT_ID_PLAYER_ID, 0);
    monData = TRUE;
...
void SetBattleTowerLinkPlayerGfx(void)
{
    ...
    for (i = 0; i < 2; i++)
    {
        if (gLinkPlayers[i].gender == MALE)
            VarSet(VAR_OBJ_GFX_ID_F - i, OBJ_EVENT_GFX_RENKO_NORMAL);
        else
