    ...
    if (gBattleMons[ctx->battlerAtk].status1 & STATUS1_SLEEP)
    {
        ...
        else
        {
            ...
            if (gBattleMons[ctx->battlerAtk].status1 & STATUS1_SLEEP)
            {
                ...
                {
                    if (IsAbilityAndRecord(ctx->battlerAtk, ctx->ability[ctx->battlerAtk], ABILITY_LUCID_DREAMING))
                    {
...
{
    if (GetBattlerAbility(ctx->battlerAtk) == ABILITY_FRETFUL && gDisableStructs[ctx->battlerAtk].truantCounter)
    {
        CancelMultiTurnMoves(ctx->battlerAtk, SKY_DROP_ATTACKCANCELLER_CHECK);
        gHitMarker |= HITMARKER_UNABLE_TO_USE_MOVE;
...
static enum MoveCanceller CancellerGhost(struct BattleContext *ctx)
{
...
static enum MoveCanceller CancellerChoiceLock(struct BattleContext *ctx)
{
    ...
    if (gChosenMove != MOVE_STRUGGLE
     ...
     && (IsHoldEffectChoice(holdEffect) || ctx->ability[ctx->battlerAtk] == ABILITY_ONI_TACTICS))
        ...
        if (gBattleMons[ctx->battlerAtk].status1 & STATUS1_SLEEP
         || ctx->ability[ctx->battlerAtk] == ABILITY_COMATOSE)
            ...
            battleScript = BattleScript_AlreadyAtFullHp;
              ...
              || ctx->ability[ctx->battlerAtk] == ABILITY_LIFESTREAM)
...
static enum MoveCanceller CancellerMultihitMoves(struct BattleContext *ctx)
{
    ...
    else if (GetMoveStrikeCount(ctx->currentMove) > 1)
    {
        ...
        else
        {
            ...
            if (gBattleMoveEffects[ctx->moveEffect].twoTurnEffect
             && !(gBattleMons[ctx->battlerAtk].volatiles.multipleTurns)
             && GetBattlerHoldEffect(ctx->battlerAtk) != HOLD_EFFECT_POWER_HERB)
                gMultiHitCounter = 0;
...
bool32 CanAbilityAbsorbMove(u32 battlerAtk, u32 battlerDef, enum Ability abilityDef, u32 move, u32 moveType, enum FunctionCallOption option)
{
    ...
    case ABILITY_VOLT_ABSORB:
        if (moveType == TYPE_WIND && GetBattlerMoveTargetType(battlerAtk, move) != MOVE_TARGET_ALL_BATTLERS)
    ...
    case ABILITY_LIGHTNING_ROD:
        if (B_REDIRECT_ABILITY_IMMUNITY >= GEN_5 && moveType == TYPE_WIND && GetBattlerMoveTargetType(battlerAtk, move) != MOVE_TARGET_ALL_BATTLERS)
        {
            effect = MOVE_ABSORBED_BY_STAT_INCREASE_ABILITY;
            statId = STAT_SPATK;
        }
...
u32 AbilityBattleEffects(u32 caseID, u32 battler, enum Ability ability, u32 special, u32 moveArg)
{
    ...
    case ABILITYEFFECT_MOVE_END: // Think contact abilities.
        ...
        {
            enum Ability abilityAtk = GetBattlerAbility(gBattlerAttacker);
            if ((GetGenConfig(GEN_CONFIG_POWDER_GRASS) < GEN_6 || !IS_BATTLER_OF_TYPE(gBattlerAttacker, TYPE_NATURE))
        ...
        case ABILITY_FOUL_AROMA:
            if (IsBattlerAlive(gBattlerAttacker)
             ...
             && RandomPercentage(RNG_FOUL_AROMA, 30))
            {
                gEffectBattler = gBattlerAttacker;
                gBattleScripting.battler = gBattlerTarget;
...
bool32 CanSetNonVolatileStatus(u32 battlerAtk, u32 battlerDef, enum Ability abilityAtk, enum Ability abilityDef, enum MoveEffect effect, enum FunctionCallOption option)
{
    ...
    case MOVE_EFFECT_PARALYSIS:
        ...
        else if (GetGenConfig(GEN_CONFIG_PARALYZE_ELECTRIC) >= GEN_6 && IS_BATTLER_OF_TYPE(battlerDef, TYPE_WIND))
        {
            battleScript = BattleScript_NotAffected;
        }
...
static enum ItemEffect TrySetEnigmaBerry(u32 battler)
{
    if (IsBattlerAlive(battler)
     ...
     && ((IsBattlerTurnDamaged(battler) && gBattleStruct->moveResultFlags[battler] & (MOVE_RESULT_SUPER_EFFECTIVE | MOVE_RESULT_EXTREMELY_EFFECTIVE)) || gBattleScripting.overrideBerryRequirements)
...
u32 ItemBattleEffects(enum ItemCaseId caseID, u32 battler)
{
    ...
    case ITEMEFFECT_TARGET:
        if (!(gBattleStruct->moveResultFlags[gBattlerTarget] & MOVE_RESULT_NO_EFFECT))
        {
            ...
            case HOLD_EFFECT_WEAKNESS_POLICY:
                if (IsBattlerAlive(battler)
                    && IsBattlerTurnDamaged(gBattlerTarget)
                    && gBattleStruct->moveResultFlags[gBattlerTarget] & (MOVE_RESULT_SUPER_EFFECTIVE | MOVE_RESULT_EXTREMELY_EFFECTIVE))
                {
                    effect = ITEM_STATS_CHANGE;
    ...
    case ITEMEFFECT_ORBS:
    {
        ...
        case HOLD_EFFECT_FROST_ORB:
            if (CanGetFrostbite(battler, battler, battlerAbility))
            {
                effect = ITEM_STATUS_CHANGE;
                gBattleMons[battler].status1 = STATUS1_FROSTBITE;
...
static inline u32 CalcMoveBasePowerAfterModifiers(struct DamageContext *ctx)
{
    ...
    if (gBattleMons[battlerAtk].volatiles.charge && moveType == TYPE_REASON)
        modifier = uq4_12_multiply(modifier, UQ_4_12(2.0));
    if (GetMoveEffect(gChosenMove) == EFFECT_ME_FIRST)
        modifier = uq4_12_multiply(modifier, UQ_4_12(1.5));
    if (IsBattlerTerrainAffected(battlerAtk, ctx->abilityAtk, ctx->holdEffectAtk, STATUS_FIELD_GRASSY_TERRAIN) && moveType == TYPE_NATURE)
        modifier = uq4_12_multiply(modifier, (B_TERRAIN_TYPE_BOOST >= GEN_8 ? UQ_4_12(1.3) : UQ_4_12(1.5)));
    if (IsBattlerTerrainAffected(battlerAtk, ctx->abilityAtk, ctx->holdEffectAtk, STATUS_FIELD_MISTY_TERRAIN) && moveType == TYPE_HEART)
        modifier = uq4_12_multiply(modifier, (B_TERRAIN_TYPE_BOOST >= GEN_8 ? UQ_4_12(1.3) : UQ_4_12(1.5)));
    if (IsBattlerTerrainAffected(battlerAtk, ctx->abilityAtk, ctx->holdEffectAtk, STATUS_FIELD_ELECTRIC_TERRAIN) && moveType == TYPE_WIND)
        modifier = uq4_12_multiply(modifier, (B_TERRAIN_TYPE_BOOST >= GEN_8 ? UQ_4_12(1.3) : UQ_4_12(1.5)));
    if (IsBattlerTerrainAffected(battlerAtk, ctx->abilityAtk, ctx->holdEffectAtk, STATUS_FIELD_PSYCHIC_TERRAIN) && moveType == TYPE_REASON)
        modifier = uq4_12_multiply(modifier, (B_TERRAIN_TYPE_BOOST >= GEN_8 ? UQ_4_12(1.3) : UQ_4_12(1.5)));
    if (IsBattlerTerrainAffected(battlerAtk, ctx->abilityAtk, ctx->holdEffectAtk, STATUS_FIELD_HOLY_TERRAIN) && moveType == TYPE_FAITH)
        modifier = uq4_12_multiply(modifier, (B_TERRAIN_TYPE_BOOST >= GEN_8 ? UQ_4_12(1.3) : UQ_4_12(1.5)));
    ...
    case ABILITY_NORMALIZE:
        if (moveType == TYPE_ILLUSION && gBattleStruct->battlerState[battlerAtk].ateBoost && GetGenConfig(GEN_CONFIG_ATE_MULTIPLIER) >= GEN_7)
            modifier = uq4_12_multiply(modifier, UQ_4_12(1.2));
        break;
...
static inline uq4_12_t CalcTypeEffectivenessMultiplierInternal(struct DamageContext *ctx, uq4_12_t modifier)
{
    ...
    else if (ctx->moveType == TYPE_EARTH && !IsBattlerGroundedInverseCheck(ctx->battlerDef, ctx->abilityDef, ctx->holdEffectDef, INVERSE_BATTLE) && !(MoveIgnoresTypeIfFlyingAndUngrounded(ctx->move)))
    {
        modifier = UQ_4_12(0.0);
...
bool32 CanMoveSkipAccuracyCalc(u32 battlerAtk, u32 battlerDef, enum Ability abilityAtk, enum Ability abilityDef, u32 move, enum FunctionCallOption option)
{
    ...
    if ((gBattleMons[battlerDef].volatiles.lockOn && gDisableStructs[battlerDef].battlerWithSureHit == battlerAtk)
     || (GetGenConfig(GEN_CONFIG_TOXIC_NEVER_MISS) >= GEN_6 && nonVolatileStatus == MOVE_EFFECT_TOXIC && IS_BATTLER_OF_TYPE(battlerAtk, TYPE_MIASMA))
