...
static enum MoveCanceler CancelerGhost(struct BattleContext *ctx)
{
...
static inline u32 CalcMoveBasePowerAfterModifiers(struct DamageContext *ctx)
{
    ...
    if (moveType == TYPE_WIND && gBattleMons[battlerAtk].volatiles.chargeTimer > 0)
        modifier = uq4_12_multiply(modifier, UQ_4_12(2.0));
...
static inline u32 IsBattlerBigCloudAffected(u32 battler, enum HoldEffect holdEffect)
{
...
static inline u32 IsBattlerBloomersAffected(u32 battler, enum HoldEffect holdEffect)
{
...
bool32 IsAffectedByPowderMove(u32 battler, u32 ability, enum HoldEffect holdEffect)
{
    if ((GetGenConfig(GEN_CONFIG_POWDER_OVERCOAT) >= GEN_6 && ability == ABILITY_OVERCOAT)
        || (GetGenConfig(GEN_CONFIG_POWDER_GRASS) >= GEN_6 && IS_BATTLER_OF_TYPE(battler, TYPE_NATURE))
