    ...
    case ABILITYEFFECT_ABSORBING: // 3
        if (move != MOVE_NONE)
        {
            ...
            case ABILITY_VOLT_ABSORB:
                if (moveType == TYPE_WIND && gBattleMoves[move].target != MOVE_TARGET_ALL_BATTLERS)
                    effect = 1;
                break;
...
static inline u32 CalcMoveBasePowerAfterModifiers(u32 move, u32 battlerAtk, u32 battlerDef, u32 moveType, bool32 updateFlags, u32 atkAbility, u32 defAbility, u32 holdEffectAtk, u32 weather)
{
    ...
    if (IsBattlerTerrainAffected(battlerDef, STATUS_FIELD_MISTY_TERRAIN) && moveType == TYPE_COSMIC)
        modifier = uq4_12_multiply(modifier, UQ_4_12(0.5));
    ...
    if (IsBattlerTerrainAffected(battlerAtk, STATUS_FIELD_PSYCHIC_TERRAIN) && moveType == TYPE_REASON)
        modifier = uq4_12_multiply(modifier, (B_TERRAIN_TYPE_BOOST >= GEN_8 ? UQ_4_12(1.3) : UQ_4_12(1.5)));
    if (moveType == TYPE_WIND && ((gFieldStatuses & STATUS_FIELD_MUDSPORT)
...
static inline void MulByTypeEffectiveness(uq4_12_t *modifier, u32 move, u32 moveType, u32 battlerDef, u32 defType, u32 battlerAtk, bool32 recordAbilities)
{
    ...
    else if ((moveType == TYPE_DARK || moveType == TYPE_ILLUSION) && defType == TYPE_GHOST && GetBattlerAbility(battlerAtk) == ABILITY_SCRAPPY && mod == UQ_4_12(0.0))
    {
        mod = UQ_4_12(1.0);
        if (recordAbilities)
...
static inline void TryNoticeIllusionInTypeEffectiveness(u32 move, u32 moveType, u32 battlerAtk, u32 battlerDef, uq4_12_t resultingModifier, u32 illusionSpecies)
{
    ...
    if (GetTypeBySpecies(illusionSpecies, 2) != GetTypeBySpecies(illusionSpecies, 1))
...
static inline uq4_12_t CalcTypeEffectivenessMultiplierInternal(u32 move, u32 moveType, u32 battlerAtk, u32 battlerDef, bool32 recordAbilities, uq4_12_t modifier, u32 defAbility)
{
    ...
    else if (moveType == TYPE_EARTH && !IsBattlerGrounded2(battlerDef, TRUE) && !(gBattleMoves[move].ignoreTypeIfFlyingAndUngrounded))
    {
        modifier = UQ_4_12(0.0);
