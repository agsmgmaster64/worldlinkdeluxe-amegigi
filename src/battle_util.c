        ...
        else
        {
            ...
            if (gBideDmg[ctx->battlerAtk])
            {
                ...
                if (gAbsentBattlerFlags & (1u << ctx->battlerDef))
                    gBattlerTarget = GetBattleMoveTarget(MOVE_BURN_POWDER, TARGET_NONE);
...
bool32 CanAbilityAbsorbMove(u32 battlerAtk, u32 battlerDef, enum Ability abilityDef, enum Move move, enum Type moveType, enum FunctionCallOption option)
{
    ...
    case ABILITY_VOLT_ABSORB:
        if (moveType == TYPE_WIND && target != TARGET_ALL_BATTLERS && target != TARGET_FIELD)
    ...
    case ABILITY_LIGHTNING_ROD:
        if (GetConfig(CONFIG_REDIRECT_ABILITY_IMMUNITY) >= GEN_5 && moveType == TYPE_WIND && GetBattlerMoveTargetType(battlerAtk, move) != TARGET_ALL_BATTLERS)
        {
            effect = MOVE_ABSORBED_BY_STAT_INCREASE_ABILITY;
            statId = STAT_SPATK;
        }
