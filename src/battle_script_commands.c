...
bool32 IsMovePowderBlocked(struct BattleContext *ctx)
{
    ...
    if (IsPowderMove(ctx->currentMove) && (ctx->battlerAtk != ctx->battlerDef))
    {
        if (GetGenConfig(GEN_CONFIG_POWDER_GRASS) >= GEN_6
         && (IS_BATTLER_OF_TYPE(ctx->battlerDef, TYPE_NATURE) || ctx->abilities[ctx->battlerDef] == ABILITY_OVERCOAT))
        {
...
s32 CalcCritChanceStage(u32 battlerAtk, u32 battlerDef, u32 move, bool32 recordAbility, enum Ability abilityAtk, enum Ability abilityDef, enum HoldEffect holdEffectAtk, enum HoldEffect holdEffectDef)
{
    s32 critChance = 0;
...
s32 CalcCritChanceStageGen1(u32 battlerAtk, u32 battlerDef, u32 move, bool32 recordAbility, enum Ability abilityAtk, enum Ability abilityDef, enum HoldEffect holdEffectAtk, enum HoldEffect holdEffectDef)
{
    s32 critChance = 0;
...
void BS_TryFlingHoldEffect(void)
{
    ...
    case HOLD_EFFECT_TYPE_POWER:
        if (GetItemSecondaryId(gLastUsedItem) != TYPE_MIASMA)
        {
            gBattlescriptCurrInstr = cmd->nextInstr;
        }
        else
