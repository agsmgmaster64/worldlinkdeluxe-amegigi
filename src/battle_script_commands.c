...
bool32 IsMovePowderBlocked(struct BattleContext *ctx)
{
    ...
    if (IsPowderMove(ctx->currentMove) && (ctx->battlerAtk != ctx->battlerDef))
    {
        if (B_POWDER_GRASS >= GEN_6
         && (IS_BATTLER_OF_TYPE(ctx->battlerDef, TYPE_NATURE) || ctx->ability[ctx->battlerDef] == ABILITY_OVERCOAT))
        {
...
static void Cmd_adjustdamage(void)
{
    ...
    for (battlerDef = 0; battlerDef < gBattlersCount; battlerDef++)
    {
        ...
        if (GetBattlerAbility(battlerDef) == ABILITY_ICE_FACE && IsBattleMovePhysical(gCurrentMove) && gBattleMons[battlerDef].species == SPECIES_EISCUE)
        {
            ...
            gBattleStruct->moveResultFlags[battlerDef] &= ~(MOVE_RESULT_SUPER_EFFECTIVE | MOVE_RESULT_NOT_VERY_EFFECTIVE | MOVE_RESULT_MOSTLY_INEFFECTIVE | MOVE_RESULT_EXTREMELY_EFFECTIVE);
...
static bool32 HandleMoveEndAbilityBlock(u32 battlerAtk, u32 battlerDef, u32 move)
{
    ...
    case ABILITY_PICKPOCKET:
        if (move != MOVE_FLING && move != MOVE_NATURAL_GIFT
         ...
         && !(gWishFutureKnock.knockedOffMons[GetBattlerSide(battlerDef)] & (1u << gBattlerPartyIndexes[battlerDef]))
         && !DoesSubstituteBlockMove(battlerAtk, battlerDef, move)
         && IsMoveMakingContact(battlerAtk, battlerDef, abilityAtk, GetBattlerHoldEffect(battlerAtk, TRUE), move)
         && (GetBattlerAbility(battlerDef) != ABILITY_COLLECTOR || !IsBattlerAlive(battlerDef)))
        {
            StealTargetItem(battlerAtk, battlerDef);
            gBattleScripting.battler = gBattlerAbility = battlerAtk;
        ...
        case MOVEEND_CONCERTO:
            ...
            if (IsSoundMove(originallyUsedMove) && !gBattleStruct->snatchedMoveIsUsed)
            {
                ...
                {   // Sound move succeeds
                    ...
                    if (nextConcerto && AbilityBattleEffects(ABILITYEFFECT_MOVE_END_OTHER, nextConcerto & 0x3, 0, 0, originallyUsedMove))
                        effect = TRUE;
                }
            }
            gBattleScripting.moveendState++;
