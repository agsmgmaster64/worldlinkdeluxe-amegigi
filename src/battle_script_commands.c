...
{
    MOVE_BIND, MOVE_WRAP, MOVE_FIRE_SPIN, MOVE_WHIRLPOOL, MOVE_SAND_TOMB, MOVE_MAGMA_STORM, MOVE_INFESTATION, MOVE_SNAP_TRAP,
};
...
static bool32 AccuracyCalcHelper(u16 move)
{
    if ((gStatuses3[gBattlerTarget] & STATUS3_ALWAYS_HITS && gDisableStructs[gBattlerTarget].battlerWithSureHit == gBattlerAttacker)
     || (B_TOXIC_NEVER_MISS >= GEN_6 && gMovesInfo[move].effect == EFFECT_TOXIC && IS_BATTLER_OF_TYPE(gBattlerAttacker, TYPE_MIASMA))
...
s32 CalcCritChanceStageArgs(u32 battlerAtk, u32 battlerDef, u32 move, bool32 recordAbility, u32 abilityAtk, u32 abilityDef, u32 holdEffectAtk)
{
    ...
    if (gSideStatuses[battlerDef] & SIDE_STATUS_LUCKY_CHANT
        || abilityDef == ABILITY_GUARD_ARMOR || abilityDef == ABILITY_SHELL_ARMOR)
    {
        critChance = -1;
    }
    ...
    else
    {
        ...
        if (critChance >= 3 && recordAbility && (abilityDef == ABILITY_GUARD_ARMOR || abilityDef == ABILITY_SHELL_ARMOR))
...
static void Cmd_switchineffects(void)
{
    ...
    else
    {
        ...
        if ((battlerAbility == ABILITY_INTIMIDATE || battlerAbility == ABILITY_FASCINATE || battlerAbility == ABILITY_SUPERSWEET_SYRUP || battlerAbility == ABILITY_DOWNLOAD)
...
bool32 CanPoisonType(u8 battlerAttacker, u8 battlerTarget)
{
    return GetBattlerAbility(battlerAttacker) == ABILITY_CORROSION
        || (!IS_BATTLER_OF_TYPE(battlerTarget, TYPE_STEEL) && !IS_BATTLER_OF_TYPE(battlerTarget, TYPE_MIASMA));
}
...
static bool32 IsElectricAbilityAffected(u32 ability)
{
    ...
    if (moveType == TYPE_WIND && GetBattlerAbility(gBattlerTarget) == ability)
        return TRUE;
    else
        return FALSE;
}
...
static void Cmd_setfocusenergy(void)
{
    ...
    else if (gMovesInfo[gCurrentMove].effect == EFFECT_DRAGON_CHEER && !IS_BATTLER_OF_TYPE(gBattlerTarget, TYPE_FAITH))
    {
...
void ApplyExperienceMultipliers(s32 *expAmount, u8 expGetterMonId, u8 faintedBattler)
{
    ...
    if (B_AFFECTION_MECHANICS == TRUE && GetBattlerAffectionHearts(expGetterMonId) >= AFFECTION_TWO_HEARTS)
        *expAmount = (*expAmount * 4915) / 4096;
