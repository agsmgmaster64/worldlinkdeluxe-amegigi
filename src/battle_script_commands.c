...
static const u16 sNaturePowerMoves[BATTLE_ENVIRONMENT_COUNT] =
{
    ...
    [BATTLE_ENVIRONMENT_GRASS]      = MOVE_ENERGY_LIGHT,
    [BATTLE_ENVIRONMENT_LONG_GRASS] = MOVE_ENERGY_LIGHT,
...
static const u8 sEnvironmentToType[BATTLE_ENVIRONMENT_COUNT] =
{
    ...
    [BATTLE_ENVIRONMENT_BUILDING]         = TYPE_ILLUSION,
    ...
    [BATTLE_ENVIRONMENT_DISTORTION_WORLD] = TYPE_ILLUSION,
    [BATTLE_ENVIRONMENT_SPACE]            = TYPE_COSMIC,
    [BATTLE_ENVIRONMENT_ULTRA_SPACE]      = TYPE_REASON,
    [BATTLE_ENVIRONMENT_MOUNTAIN]         = (B_CAMOUFLAGE_TYPES >= GEN_5 ? TYPE_EARTH : TYPE_BEAST),
    [BATTLE_ENVIRONMENT_PLAIN]            = (B_CAMOUFLAGE_TYPES >= GEN_4 ? TYPE_EARTH : TYPE_ILLUSION),
};
...
static bool32 AccuracyCalcHelper(u32 move, u32 battler)
{
    ...
    if ((gStatuses3[battler] & STATUS3_ALWAYS_HITS && gDisableStructs[battler].battlerWithSureHit == gBattlerAttacker)
     || (B_TOXIC_NEVER_MISS >= GEN_6 && moveEffect == EFFECT_TOXIC && IS_BATTLER_OF_TYPE(gBattlerAttacker, TYPE_MIASMA))
...
u32 GetTotalAccuracy(u32 battlerAtk, u32 battlerDef, u32 move, u32 atkAbility, u32 defAbility, u32 atkHoldEffect, u32 defHoldEffect)
{
    ...
    if (atkAbility == ABILITY_UNAWARE || atkAbility == ABILITY_KEEN_EYE || atkAbility == ABILITY_MINDS_EYE
            || (B_ILLUMINATE_EFFECT >= GEN_9 && atkAbility == ABILITY_DIVA))
...
static inline u32 IsBattlerBigCloudAffected(u32 battler, enum ItemHoldEffect holdEffect)
{
...
static inline u32 IsBattlerBloomersAffected(u32 battler, enum ItemHoldEffect holdEffect)
{
...
s32 CalcCritChanceStage(u32 battlerAtk, u32 battlerDef, u32 move, bool32 recordAbility, u32 abilityAtk, u32 abilityDef, enum ItemHoldEffect holdEffectAtk, enum ItemHoldEffect holdEffectDef)
{
    s32 critChance = 0;
...
s32 CalcCritChanceStageGen1(u32 battlerAtk, u32 battlerDef, u32 move, bool32 recordAbility, u32 abilityAtk, u32 abilityDef, enum ItemHoldEffect holdEffectAtk, enum ItemHoldEffect holdEffectDef)
{
    s32 critChance = 0;
        ...
        else
        {
            ...
            case MOVE_EFFECT_CONFUSION:
                ...
                else
                {
                    ...
                    if (gMovesInfo[gCurrentMove].effect == EFFECT_SKY_DROP)
                    {
            ...
            if (stats & 1)
            {
                if (flags & STAT_CHANGE_CANT_PREVENT)
                {
                    if (gBattleMons[battler].statStages[currStat] > MIN_STAT_STAGE)
                    {
                        ...
                        && !(B_ILLUMINATE_EFFECT >= GEN_9 && ability == ABILITY_DIVA && currStat == STAT_ACC)
    ...
    case ABILITY_BEAST_BOOST:
        {
            ...
            else if (abilityAtk == ABILITY_AMBITION)
                stat = STAT_SPATK;
...
static void Cmd_moveend(void)
{
    ...
    do
    {
             ...
             && !(moveEffect == EFFECT_PRESENT && gBattleStruct->presentBasePower == 0)) // Silly edge case
            {
                ...
                else
                {
                    ...
                    if (gBattleMons[gBattlerAttacker].hp
                     ...
                     && (gMovesInfo[gChosenMove].effect == EFFECT_SLEEP_TALK || (gMovesInfo[gChosenMove].effect == EFFECT_SNORE) || !(gBattleMons[gBattlerAttacker].status1 & STATUS1_SLEEP))
        ...
        case MOVEEND_CLEAR_BITS: // Clear/Set bits for things like using a move for all targets and all hits.
            ...
            if (B_CHARGE >= GEN_9 && moveType == TYPE_WIND && (IsBattlerTurnDamaged(gBattlerTarget) || gBattleStruct->moveResultFlags[gBattlerTarget] & MOVE_RESULT_NO_EFFECT))
        ...
        case MOVEEND_CONCERTO: // Special case because it's so annoying
            if (IsSoundMove(gCurrentMove) && !gBattleStruct->snatchedMoveIsUsed)
            {
                u32 battler, nextConcerto = 0;
...
static bool32 DoSwitchInEffectsForBattler(u32 battler)
{
        ...
        && IsBattlerGrounded(battler))
    {
        ...
        if (IS_BATTLER_OF_TYPE(battler, TYPE_MIASMA)) // Absorb the toxic spikes.
        {
            gSideStatuses[GetBattlerSide(battler)] &= ~SIDE_STATUS_TOXIC_SPIKES;
    ...
    else
    {
        ...
        if (battlerAbility == ABILITY_FRETFUL
...
static u32 ChangeStatBuffs(s8 statValue, u32 statId, u32 flags, const u8 *BS_ptr)
{
    ...
    if (statValue <= -1) // Stat decrease.
    {
        if (gSideTimers[GetBattlerSide(battler)].mistTimer
            && !certain && moveEffect != EFFECT_CURSE
        ...
        else if (moveEffect != EFFECT_CURSE
                 && notProtectAffected != TRUE && JumpIfMoveAffectedByProtect(gCurrentMove, gBattlerTarget, TRUE))
        {
            ...
            return STAT_CHANGE_DIDNT_WORK;
              ...
              && (!affectsUser || mirrorArmored) && !certain && moveEffect != EFFECT_CURSE)
        {
            if (flags == STAT_CHANGE_ALLOW_PTR)
            {
                if (gSpecialStatuses[battler].statLowered)
        ...
        else if ((index = IsFlowerVeilProtected(battler)) && !certain)
        {
            ...
            return STAT_CHANGE_DIDNT_WORK;
                ...
                || (B_ILLUMINATE_EFFECT >= GEN_9 && battlerAbility == ABILITY_DIVA && statId == STAT_ACC)
    ...
    else
    {
        ...
        else
        {
            ...
            case BALL_SAFARI:
                ...
                break;
            case BALL_NET:
                if (IS_BATTLER_ANY_TYPE(gBattlerTarget, TYPE_WATER, TYPE_BEAST))
...
void BS_TryEarlyBirdHeal(void)
{
    ...
    if (battlerAbility == ABILITY_EARLY_BIRD && !IsBattlerAtMaxHp(battler) && !(gStatuses3[battler] & STATUS3_HEAL_BLOCK))
    {
        gBattleStruct->moveDamage[battler] = GetNonDynamaxMaxHP(battler) / 4;
        if (gBattleStruct->moveDamage[battler] == 0)
            gBattleStruct->moveDamage[battler] = 1;
