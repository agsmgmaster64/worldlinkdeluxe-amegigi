...
static const u8 sStatsLeftColumnLayoutEV[] = _("EVs:{DYNAMIC 0}\n{DYNAMIC 1}\n{DYNAMIC 2}");
static const u8 sStatsLeftColumnLayoutIV[] = _("IVs:{DYNAMIC 0}\n{DYNAMIC 1}\n{DYNAMIC 2}");
...
static void ChangeSummaryState (s16 *taskData, u8 taskId)
{
    switch (taskData[3])
    {
        case 0:
            ...
            break;
        case 1:
            if (gSaveBlock2Ptr->optionsSummaryIvView == 0)
                taskData[3] = 2;
            else
                taskData[3] = 0;
            break;
...
static void Task_HandleInput(u8 taskId)
{
    ...
    if (MenuHelpers_ShouldWaitForLinkRecv() != TRUE && !gPaletteFade.active)
    {
        ...
        else if (DEBUG_POKEMON_SPRITE_VISUALIZER && JOY_NEW(SELECT_BUTTON) && !gMain.inBattle && gSaveBlock2Ptr->optionsDebugMode == 0)
        {
...
//    PrintTextOnWindow(PSS_LABEL_WINDOW_POKEMON_SKILLS_STATS_RIGHT, gText_Speed2, statsXPos, 33, 0, 1);
    ...
    PrintTextOnWindowWithFont(PSS_LABEL_WINDOW_PROMPT_RELEARN, gText_Relearn, 0, 4, 0, 13, FONT_SMALL);
}
...
static void BufferIvOrEvStats(u8 mode)
{
    ...
    case 0:
        ...
        ConvertIntToDecimalStringN(gStringVar1, maxHp, STR_CONV_MODE_RIGHT_ALIGN, 3);
...
static u8 LoadMonGfxAndSprite(struct Pokemon *mon, s16 *state)
{
    ...
    case 0:
        ...
        return 0xFF;
    case 1:
        LoadCompressedUniqueSpritePaletteWithTag(GetMonSpritePalFromSpeciesAndPersonality(summary->species2, summary->isShiny, summary->pid), summary->species2, summary->pid, summary->isShiny);
