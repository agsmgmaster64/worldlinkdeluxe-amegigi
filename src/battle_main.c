...
{
    [TRAINER_CLASS_COOLTRAINER] = ITEM_ULTRA_ORB,
    [TRAINER_CLASS_COLLECTOR] = ITEM_PREMIER_ORB,
    [TRAINER_CLASS_SWIMMER_M] = ITEM_DIVE_ORB,
    [TRAINER_CLASS_BLACK_BELT] = ITEM_ULTRA_ORB,
    [TRAINER_CLASS_AQUA_LEADER] = ITEM_MASTER_ORB,
    [TRAINER_CLASS_GENTLEMAN] = ITEM_LUXURY_ORB,
    [TRAINER_CLASS_ELITE_FOUR] = ITEM_ULTRA_ORB,
#if B_TRAINER_CLASS_POKE_BALLS == GEN_7
    [TRAINER_CLASS_FISHERMAN] = ITEM_LURE_BALL,
#elif B_TRAINER_CLASS_POKE_BALLS == GEN_8
    [TRAINER_CLASS_FISHERMAN] = ITEM_DIVE_ORB,
#endif
    [TRAINER_CLASS_SWIMMER_F] = ITEM_DIVE_ORB,
    [TRAINER_CLASS_COOLTRAINER_2] = ITEM_ULTRA_ORB,
    [TRAINER_CLASS_MAGMA_LEADER] = ITEM_MASTER_ORB,
};
#endif
...
u8 CreateNPCTrainerPartyFromTrainer(struct Pokemon *party, const struct Trainer *trainer, bool32 firstTrainer, u32 battleTypeFlags)
{
                                                                        ...
                                                                        | BATTLE_TYPE_TRAINER_HILL)))
    {
        ...
        for (i = 0; i < monsCount; i++)
        {
            ...
            if (ball == -1)
            {
                ball = (sTrainerBallTable[trainer->trainerClass]) ? sTrainerBallTable[trainer->trainerClass] : ITEM_TOHO_ORB;
                SetMonData(&party[i], MON_DATA_POKEBALL, &ball);
            }
        #endif
        }
...
void SetTypeBeforeUsingMove(u32 move, u32 battlerAtk)
{
    ...
    if (gBattleMoves[move].effect == EFFECT_WEATHER_BALL)
    {
        if (WEATHER_HAS_EFFECT)
        {
            if (gBattleWeather & B_WEATHER_RAIN && holdEffect != HOLD_EFFECT_UTILITY_UMBRELLA)
                ...
                gBattleStruct->dynamicMoveType = TYPE_EARTH | F_DYNAMIC_TYPE_2;
    ...
    else if (gBattleMoves[move].effect == EFFECT_HIDDEN_POWER)
    {
        ...
        // Subtract 4 instead of 1 below because 3 types are excluded (TYPE_ILLUSION and TYPE_MYSTERY and TYPE_COSMIC)
    ...
    else if (gBattleMoves[move].effect == EFFECT_TERRAIN_PULSE)
    {
        if (IsBattlerTerrainAffected(battlerAtk, STATUS_FIELD_TERRAIN_ANY))
        {
            if (gFieldStatuses & STATUS_FIELD_ELECTRIC_TERRAIN)
                gBattleStruct->dynamicMoveType = TYPE_WIND | F_DYNAMIC_TYPE_2;
            else if (gFieldStatuses & STATUS_FIELD_GRASSY_TERRAIN)
                gBattleStruct->dynamicMoveType = TYPE_NATURE | F_DYNAMIC_TYPE_2;
            else if (gFieldStatuses & STATUS_FIELD_MISTY_TERRAIN)
                gBattleStruct->dynamicMoveType = TYPE_COSMIC | F_DYNAMIC_TYPE_2;
            else if (gFieldStatuses & STATUS_FIELD_PSYCHIC_TERRAIN)
                gBattleStruct->dynamicMoveType = TYPE_REASON | F_DYNAMIC_TYPE_2;
            else //failsafe
                gBattleStruct->dynamicMoveType = TYPE_ILLUSION | F_DYNAMIC_TYPE_2;
        }
    }
    ...
    else if (gStatuses4[battlerAtk] & STATUS4_PLASMA_FISTS && moveType == TYPE_ILLUSION)
    {
        gBattleStruct->dynamicMoveType = TYPE_WIND | F_DYNAMIC_TYPE_2;
    }
