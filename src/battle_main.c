...
const struct TrainerClass gTrainerClasses[TRAINER_CLASS_COUNT] =
{
    ...
    TRAINER_CLASS(PKMN_BREEDER, "{PKMN} Breeder", 10),
    TRAINER_CLASS(COOLTRAINER, "Ace Trainer", 12, ITEM_ULTRA_ORB),
    TRAINER_CLASS(BIRD_KEEPER, "Bird Keeper", 8),
    TRAINER_CLASS(COLLECTOR, "Collector", 15, ITEM_PREMIER_ORB),
    TRAINER_CLASS(SWIMMER_M, "Swimmer♂", 2, ITEM_DIVE_ORB),
    TRAINER_CLASS(TEAM_MAGMA, "Team Magma"),
    TRAINER_CLASS(EXPERT, "Expert", 10),
    TRAINER_CLASS(AQUA_ADMIN, "Aqua Admin", 10),
    TRAINER_CLASS(BLACK_BELT, "Black Belt", 8, ITEM_ULTRA_ORB),
    TRAINER_CLASS(AQUA_LEADER, "Aqua Leader", 20, ITEM_MASTER_ORB),
    ...
    TRAINER_CLASS(BUG_MANIAC, "Fairy Maniac", 15),
    TRAINER_CLASS(PSYCHIC, "Psychic", 6),
    TRAINER_CLASS(GENTLEMAN, "Gentleman", 20, ITEM_LUXURY_ORB),
    TRAINER_CLASS(ELITE_FOUR, "Elite Four", 25, ITEM_ULTRA_ORB),
...
{
    u32 moveType, attackerAbility, multiPulseType;
    ...
    if (gMovesInfo[move].effect == EFFECT_WEATHER_BALL)
    {
        if (WEATHER_HAS_EFFECT)
        {
            if (gBattleWeather & B_WEATHER_RAIN && holdEffect != HOLD_EFFECT_UTILITY_UMBRELLA)
                ...
                gBattleStruct->dynamicMoveType = TYPE_EARTH | F_DYNAMIC_TYPE_SET;
    ...
    else if (gMovesInfo[move].effect == EFFECT_REVELATION_DANCE && GetActiveGimmick(battlerAtk) != GIMMICK_Z_MOVE)
    {
        if (GetActiveGimmick(battlerAtk) == GIMMICK_TERA && GetBattlerTeraType(battlerAtk) != TYPE_STELLAR)
            ...
            gBattleStruct->dynamicMoveType = (B_ROOST_PURE_FLYING >= GEN_5 ? TYPE_ILLUSION : TYPE_MYSTERY) | F_DYNAMIC_TYPE_SET;
