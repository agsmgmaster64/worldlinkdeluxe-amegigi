...
static void CursorCb_Misc(u8 taskId)
{
    PlaySE(SE_SELECT);
    PartyMenuRemoveWindow(&sPartyMenuInternal->windowId[0]);
    PartyMenuRemoveWindow(&sPartyMenuInternal->windowId[1]);
    SetPartyMonSelectionActions(gPlayerParty, gPartyMenu.slotId, ACTIONS_MISC);
...
static bool8 NotUsingHPEVItemOn1HPMon(struct Pokemon *mon, u16 item)
{
...
{
    MainCallback callback = (InBattlePyramid() || FlagGet(FLAG_USE_PYRAMID_BAG)) ? CB2_ReturnToPyramidBagMenu : CB2_ReturnToBagMenu;
