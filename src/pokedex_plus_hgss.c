...
static const u8 sText_EVO_ITEM_LEVEL[] = _("{STR_VAR_2} is used, Evo level");
...
static u8 ShowCategoryIcon(u32 category)
{
    if (sPokedexView->categoryIconSpriteId == 0xFF)
        sPokedexView->categoryIconSpriteId = CreateSprite(&sSpriteTemplate_CategoryIcons, 174, 43, 0);
...
static u8 PrintEvolutionTargetSpeciesAndMethod(u8 taskId, u16 species, u8 depth, u8 depth_i)
{
    ...
    for (i = 0; i < times; i++)
    {
        ...
        case EVO_ITEM_LEVEL:
            item = evolutions[i].param;
            CopyItemName(item, gStringVar2);
            StringExpandPlaceholders(gStringVar4, sText_EVO_ITEM_LEVEL );
            break;
...
static void Task_StartPokedexSearch(u8 taskId)
{
    ...
    u8 sourceGame = GetSearchModeSelection(taskId, SEARCH_COLOR);
