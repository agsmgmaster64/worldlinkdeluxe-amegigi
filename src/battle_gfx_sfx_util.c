...
void BattleLoadMonSpriteGfx(struct Pokemon *mon, u32 battler)
{
    ...
    UniquePalette(paletteOffset, species, personalityValue, isShiny);
    CpuCopy32(gPlttBufferFaded + paletteOffset, gPlttBufferUnfaded + paletteOffset, 32);
    UniquePalette(0x80 + battler * 16, species, personalityValue, isShiny);
...
void HandleSpeciesGfxDataChange(u8 battlerAtk, u8 battlerDef, u32 transformType, bool8 trackEnemyPersonality)
{
    ...
    else if (transformType == 255)
    {
        ...
        void *bufferGhost = malloc_and_decompress(lzPaletteData, NULL);
        LoadPalette(bufferGhost, paletteOffset, PLTT_SIZE_4BPP);
    ...
    else
    {
        ...
        else
        {
            ...
            if (B_TRANSFORM_SHINY >= GEN_4 && trackEnemyPersonality && !transformType)
            {
