...
static const u16 sMuddySlopeMetatiles[] = {
    METATILE_RG_General_MuddySlope_Frame0,
    METATILE_RG_General_MuddySlope_Frame3,
    METATILE_RG_General_MuddySlope_Frame2,
    METATILE_RG_General_MuddySlope_Frame1
};
...
static void SetMuddySlopeMetatile(s16 *data, s16 x, s16 y)
{
    ...
    if ((--data[SLOPE_TIME]) == 0)
        metatileId = METATILE_RG_General_MuddySlope_Frame0;
    else
        metatileId = sMuddySlopeMetatiles[data[SLOPE_TIME] / SLOPE_ANIM_STEP_TIME];

    MapGridSetMetatileIdAt(x, y, metatileId);
    CurrentMapDrawMetatileAt(x, y);
    MapGridSetMetatileIdAt(x, y, METATILE_RG_General_MuddySlope_Frame0);
}
