    ...
    if (WEATHER_HAS_EFFECT)
    {
        ...
        if (holdEffect != HOLD_EFFECT_SAFETY_GOGGLES && ability != ABILITY_MAGIC_GUARD && ability != ABILITY_OVERCOAT)
        {
             ...
             && ability != ABILITY_SNOW_CLOAK && ability != ABILITY_ICE_BODY)
            {
                ...
                && (AI_DATA->switchinCandidate.battleMon.type1 != TYPE_EARTH && AI_DATA->switchinCandidate.battleMon.type2 != TYPE_EARTH
...
static u32 GetSwitchinRecurringHealing(void)
{
    ...
    {
        if (holdEffect == HOLD_EFFECT_BLACK_SLUDGE && (AI_DATA->switchinCandidate.battleMon.type1 == TYPE_MIASMA || AI_DATA->switchinCandidate.battleMon.type2 == TYPE_MIASMA))
        {
