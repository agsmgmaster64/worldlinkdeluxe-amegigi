...
static bool32 ShouldSwitchIfHasBadOdds(u32 battler)
{
    ...
    for (i = 0; i < MAX_MON_MOVES; i++)
    {
        ...
        if (aiMove != MOVE_NONE)
        {
            ...
            || aiMoveEffect == EFFECT_SLEEP || aiMoveEffect == EFFECT_YAWN || aiMoveEffect == EFFECT_TOXIC || aiMoveEffect == EFFECT_WILL_O_WISP || aiMoveEffect == EFFECT_PARALYZE || aiMoveEffect == EFFECT_SUB_ZERO
...
static bool32 FindMonThatAbsorbsOpponentsMove(u32 battler)
{
    ...
    else if (incomingType == TYPE_WIND || (isOpposingBattlerChargingOrInvulnerable && incomingType == TYPE_WIND))
    {
    ...
    else if (incomingType == TYPE_NATURE || (isOpposingBattlerChargingOrInvulnerable && incomingType == TYPE_NATURE))
    {
    ...
    if (gAiThinkingStruct->aiFlags[GetThinkingBattler(battler)] & AI_FLAG_SMART_SWITCHING)
    {
            ...
            && RandomPercentage(RNG_AI_SWITCH_YAWN, GetSwitchChance(SHOULD_SWITCH_YAWN)))
        {
            ...
            if (gBattleMons[battler].statStages[STAT_EVASION] > (DEFAULT_STAT_STAGE + 3)
                ...
                && (B_ILLUMINATE_EFFECT >= GEN_9 && gAiLogicData->abilities[opposingBattler] != ABILITY_DIVA)
    ...
    if (HasWeatherEffect())
    {
        ...
        if (holdEffect != HOLD_EFFECT_SAFETY_GOGGLES && ability != ABILITY_MAGIC_GUARD && ability != ABILITY_OVERCOAT)
        {
             ...
             && ability != ABILITY_SNOW_CLOAK && ability != ABILITY_ICE_BODY)
            {
                ...
                && (gAiLogicData->switchinCandidate.battleMon.types[0] != TYPE_EARTH && gAiLogicData->switchinCandidate.battleMon.types[1] != TYPE_EARTH
...
static u32 GetSwitchinRecurringHealing(void)
{
    ...
    {
        if (holdEffect == HOLD_EFFECT_BLACK_SLUDGE && (gAiLogicData->switchinCandidate.battleMon.types[0] == TYPE_MIASMA || gAiLogicData->switchinCandidate.battleMon.types[1] == TYPE_MIASMA))
        {
...
static u32 GetBestMonIntegrated(struct Pokemon *party, int firstId, int lastId, u32 battler, u32 opposingBattler, u32 battlerIn1, u32 battlerIn2, enum SwitchType switchType)
{
    ...
    for (i = firstId; i < lastId; i++)
    {
        ...
        if (gAiLogicData->switchinCandidate.battleMon.ability == ABILITY_FRETFUL && IsTruantMonVulnerable(battler, opposingBattler))
            continue;
    ...
    else
    {
        ...
        for (i = firstId; i < lastId; i++)
        {
            if (!IsValidForBattle(&party[i])
                ...
                || i == gBattleStruct->monToSwitchIntoId[battlerIn1]
                || i == gBattleStruct->monToSwitchIntoId[battlerIn2]
                || (GetMonAbility(&party[i]) == ABILITY_FRETFUL && IsTruantMonVulnerable(battler, opposingBattler))) // While not really invalid per se, not really wise to switch into this mon.
            {
                invalidMons |= 1u << i;
            }
