...
static bool32 FindMonThatHitsWonderGuard(u32 battler)
{
    ...
    if (AI_DATA->abilities[opposingBattler] != ABILITY_PLAY_GHOST)
        return FALSE;
...
static bool32 FindMonThatAbsorbsOpponentsMove(u32 battler)
{
    ...
    else if (gMovesInfo[predictedMove].type == TYPE_WIND || (isOpposingBattlerChargingOrInvulnerable && gMovesInfo[incomingMove].type == TYPE_WIND))
    {
    ...
    else if (gMovesInfo[predictedMove].type == TYPE_NATURE || (isOpposingBattlerChargingOrInvulnerable && gMovesInfo[incomingMove].type == TYPE_NATURE))
    {
    ...
    if (AI_THINKING_STRUCT->aiFlags[battler] & AI_FLAG_SMART_SWITCHING)
    {
            ...
            && gBattleMons[battler].hp > gBattleMons[battler].maxHP / 3)
        {
            ...
            if (gBattleMons[battler].statStages[STAT_EVASION] > (DEFAULT_STAT_STAGE + 3)
                ...
                && (B_ILLUMINATE_EFFECT >= GEN_9 && AI_DATA->abilities[opposingBattler] != ABILITY_DIVA)
    ...
    if (WEATHER_HAS_EFFECT)
    {
        ...
        if (holdEffect != HOLD_EFFECT_SAFETY_GOGGLES && ability != ABILITY_MAGIC_GUARD && ability != ABILITY_OVERCOAT)
        {
             ...
             && ability != ABILITY_SNOW_CLOAK && ability != ABILITY_ICE_BODY)
            {
                ...
                && (AI_DATA->switchinCandidate.battleMon.types[0] != TYPE_EARTH && AI_DATA->switchinCandidate.battleMon.types[1] != TYPE_EARTH
...
static u32 GetSwitchinRecurringHealing(void)
{
    ...
    {
        if (holdEffect == HOLD_EFFECT_BLACK_SLUDGE && (AI_DATA->switchinCandidate.battleMon.types[0] == TYPE_MIASMA || AI_DATA->switchinCandidate.battleMon.types[1] == TYPE_MIASMA))
        {
