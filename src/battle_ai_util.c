...
void SetBattlerData(u32 battlerId)
{
    if (!BattlerHasAi(battlerId))
    {
        ...
        else if (GetAbilityBySpecies(species, 1) == ABILITY_NONE
                || GetAbilityBySpecies(species, 1) == GetAbilityBySpecies(species, 0))
...
static bool32 AI_IsMoveEffectInPlus(u32 battlerAtk, u32 battlerDef, u32 move, s32 noOfHitsToKo)
{
    ...
    case EFFECT_ATTACK_DOWN_HIT:
        if (ShouldLowerStat(battlerDef, abilityDef, STAT_ATK) && abilityDef != ABILITY_HIGH_STRENGTH && noOfHitsToKo != 1)
            return TRUE;
        break;
...
bool32 IsMoveEncouragedToHit(u32 battlerAtk, u32 battlerDef, u32 move)
{
    ...
    if (B_TOXIC_NEVER_MISS >= GEN_6 && gBattleMoves[move].effect == EFFECT_TOXIC && IS_BATTLER_OF_TYPE(battlerAtk, TYPE_MIASMA))
        return TRUE;
...
bool32 ShouldPoisonSelf(u32 battler, u32 ability)
{
    if (AI_CanBePoisoned(battler, battler, 0) && (
     ability == ABILITY_SPRING_CHARM
      ...
      || (ability == ABILITY_UNWAVERING && HasMoveWithCategory(battler,BATTLE_CATEGORY_SPECIAL))
...
bool32 AI_CanPoison(u32 battlerAtk, u32 battlerDef, u32 defAbility, u32 move, u32 partnerMove)
{
    if (!AI_CanBePoisoned(battlerAtk, battlerDef, move)
      ...
      || PartnerMoveEffectIsStatusSameTarget(BATTLE_PARTNER(battlerAtk), battlerDef, partnerMove))
        return FALSE;
    else if (defAbility != ABILITY_CORROSION && (IS_BATTLER_OF_TYPE(battlerDef, TYPE_MIASMA) || IS_BATTLER_OF_TYPE(battlerDef, TYPE_STEEL)))
        return FALSE;
...
{
    if (((AI_DATA->abilities[battlerAtk] != ABILITY_MOLD_BREAKER && (defAbility == ABILITY_ADVENT || defAbility == ABILITY_INNER_FOCUS))
