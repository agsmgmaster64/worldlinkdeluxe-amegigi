...
bool32 IsMoveEncouragedToHit(u32 battlerAtk, u32 battlerDef, u32 move)
{
    ...
    if (B_TOXIC_NEVER_MISS >= GEN_6
        ...
        && IS_BATTLER_OF_TYPE(battlerAtk, TYPE_MIASMA))
        return TRUE;
...
bool32 ShouldSetSandstorm(u32 battler, u32 ability, enum ItemHoldEffect holdEffect)
{
    ...
    if (ability == ABILITY_SAND_VEIL
     ...
     || HasMoveWithEffect(battler, EFFECT_PREDATOR_STALK)
...
bool32 CanKnockOffItem(u32 battler, u32 item)
{
    ...
    if (gAiLogicData->abilities[battler] == ABILITY_COLLECTOR)
        return FALSE;
...
bool32 IsBattlerIncapacitated(u32 battler, u32 ability)
{
    ...
    if (gBattleMons[battler].volatiles.recharge || (ability == ABILITY_FRETFUL && gDisableStructs[battler].truantCounter != 0))
        return TRUE;
