...
AI_SINGLE_BATTLE_TEST("AI gets baited by Protect Switch tactics") // This behavior is to be fixed.
{
    GIVEN {
        ...
        OPPONENT(SPECIES_ADVENT_MOKOU) { Moves(MOVE_TACKLE, MOVE_PECK, MOVE_EARTHQUAKE, MOVE_THUNDERBOLT); }
...
AI_SINGLE_BATTLE_TEST("AI_FLAG_SMART_MON_CHOICES: AI will not switch in a Pokemon which is slower and gets 1HKOed after fainting")
{
    ...
    GIVEN {
        ...
        OPPONENT(SPECIES_CHIBI_LUNASA) { Speed(200); Moves(MOVE_MANA_BURST, MOVE_DISABLE, MOVE_TAUNT, MOVE_CALM_MIND); }
        OPPONENT(SPECIES_NORMAL_LUNASA) { Speed(speedAlakazm); Moves(MOVE_FOCUS_BLAST, MOVE_MANA_BURST); } // Alakazam has a move which OHKOes Weavile, but it doesn't matter if he's getting KO-ed first.
        OPPONENT(SPECIES_CHIBI_DAIYOUSEI) { Speed(200); Moves(MOVE_BUBBLE_BEAM, MOVE_WATER_GUN, MOVE_LEER, MOVE_STRENGTH); } // Can't OHKO, but survives a hit from Weavile's Night Slash.
    } WHEN {
...
AI_DOUBLE_BATTLE_TEST("AI will not try to switch for the same pokemon for 2 spots in a double battle")
{
    ...
    GIVEN {
        ...
        OPPONENT(SPECIES_ATTACK_RAN) { Moves(MOVE_SHADOW_BALL); }
        OPPONENT(SPECIES_NORMAL_RAN) { Moves(MOVE_SHADOW_BALL); }
        OPPONENT(SPECIES_ATTACK_RAN) { Moves(MOVE_SHADOW_BALL); }
