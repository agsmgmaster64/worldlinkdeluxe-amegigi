...
ASSUMPTIONS
{
    ...
    ASSUME(gMovesInfo[MOVE_TACKLE].type != TYPE_EARTH);
...
{
    GIVEN {
        PLAYER(SPECIES_CHIBI_YUUGI) { Item(ITEM_AIR_BALLOON); };
...
{
    GIVEN {
        PLAYER(SPECIES_CHIBI_YUUGI) { Item(ITEM_AIR_BALLOON); MaxHP(100); HP(1); };
        OPPONENT(SPECIES_CHIBI_YUUGI);
    } WHEN {
