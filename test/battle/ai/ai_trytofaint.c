...
AI_SINGLE_BATTLE_TEST("AI prefers priority moves if it's slower and can kill target")
{
    GIVEN {
        ...
        PLAYER(SPECIES_CHIBI_YUUGI) { HP(1); Speed(100); }
        PLAYER(SPECIES_CHIBI_YUUGI) { Speed(100); }
        OPPONENT(SPECIES_CHIBI_YUUGI) { Speed(1); Moves(MOVE_QUICK_ATTACK, MOVE_STRENGTH); }
    } WHEN {
...
AI_SINGLE_BATTLE_TEST("AI will choose a random move if it's faster and can kill target")
{
    GIVEN {
        ASSUME(GetMovePriority(MOVE_QUICK_ATTACK) == 1);
        AI_FLAGS(AI_FLAG_CHECK_BAD_MOVE | AI_FLAG_CHECK_VIABILITY | AI_FLAG_TRY_TO_FAINT);
        PLAYER(SPECIES_CHIBI_YUUGI) { HP(1); Speed(1); }
        PLAYER(SPECIES_CHIBI_YUUGI) { Speed(1); }
        OPPONENT(SPECIES_CHIBI_YUUGI) { Speed(100); Moves(MOVE_QUICK_ATTACK, MOVE_STRENGTH); }
    } WHEN {
...
AI_SINGLE_BATTLE_TEST("AI will choose a priority move if it is slower then the target and will be killed")
{
    GIVEN {
        ...
        AI_FLAGS(AI_FLAG_CHECK_BAD_MOVE | AI_FLAG_CHECK_VIABILITY | AI_FLAG_TRY_TO_FAINT);
        PLAYER(SPECIES_CHIBI_YUUGI) { Speed(100); }
        OPPONENT(SPECIES_CHIBI_YUUGI) { HP(60); Speed(1); Moves(MOVE_QUICK_ATTACK, MOVE_STRENGTH); }
    } WHEN {
