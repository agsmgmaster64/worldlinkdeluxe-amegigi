...
AI_SINGLE_BATTLE_TEST("AI prefers to set up a powerful Status over fainting a target")
{
    GIVEN {
        ...
        OPPONENT(SPECIES_NORMAL_TOKIKO) { Moves(MOVE_TRICK_ROOM, MOVE_TACKLE); }
    } WHEN {
...
AI_SINGLE_BATTLE_TEST("AI will try to do damage on target instead of setting up hazards if target has a way to remove them")
{
    GIVEN {
        ...
        PLAYER(SPECIES_CHIBI_YUUGI) { HP(1); Moves(MOVE_RAPID_SPIN, MOVE_DEFOG, MOVE_CELEBRATE); }
        PLAYER(SPECIES_NORMAL_KOSUZU);
        OPPONENT(SPECIES_NORMAL_RIN) { Moves(MOVE_STEALTH_ROCK, MOVE_TACKLE); }
...
AI_SINGLE_BATTLE_TEST("AI will not set up Rain if it is already raining")
{
    GIVEN {
        ...
        PLAYER(SPECIES_NORMAL_KOSUZU);
        OPPONENT(SPECIES_CHIBI_YUUGI) { Moves(MOVE_RAIN_DANCE, MOVE_TACKLE); }
        OPPONENT(SPECIES_NORMAL_KOSUZU);
    } WHEN {
